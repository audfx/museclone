    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (var _ = Profiler.Scope("Initialize Logger Implementations"))
            {
                ...
                Logger.AddLogger(new FileLoggerImpl("msc-log.txt"));
            }
            ...
            var resourceLocator = new ClientResourceLocator("skins/user-custom", "materials/basic");
            using var host = Host.GetSuitableHost(resourceLocator);
            host.Initialize();

            using (var _ = Profiler.Scope("Chart Entity Type Registration"))
            {
                Entity.RegisterTypesFromGameMode(MusecloneGameMode.Instance);
            }

            var client = new MscClient();
